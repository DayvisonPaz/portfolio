...
{/* header, e carrinho aqui*/}
  <header><div className="flex w-screen overflow-hidden fixed h-10 justify-center items-center text-white bg-indigo-900 ">LOJA DA COMUNIDADE</div> 
 <img onClick={()=>{Setviewcart(1)}} className='fixed right-7 w-8' src="https://cdn-icons-png.flaticon.com/512/1170/1170576.png" alt="" /><p className='fixed h-6 text-center w-6 top-2 right-2 rounded-full bg-red-500'>{cart.length}</p></header>  
...
{viewcart>0?<span className='bg-indigo-900 w-1/4 fixed mt-10 h-3/6 overflow-y-scroll text-white right-4'>{cart.length>0?cart.map((e)=> <div key={e.id} className='bg-black h-1/4 flex'><img className='m-4' src={e.backdrop} alt="" /> <div className='w-3/6'><p className=''>{e.name}</p><p>{e.value} - x{e.total}</p></div><img onClick={()=>excluir(e.id)} className='w-6 h-6 mr-4 mt-5 cursor-pointer ' src="https://cdn-icons.flaticon.com/png/512/5028/premium/5028066.png?token=exp=1650274686~hmac=dba0945408344560f629d8be22c1ccba" alt="" /> <p className='h-full p-4  w-1/4 bg-green-700'>{(e.total*e.value).toFixed(2)}</p></div>):''}<div className='fixed bg-red-500 top-80 mt-5 w-1/4 h-10'>total:{total} </div></span>:''}

   {view.length>0?<section className="fixed  right-1/3 top-20  w-1/3 h-3/3 bg-gray-200">
   <img onClick={()=>Setview([])} className='right-0 m-1 absolute w-4 cursor-pointer ' src="https://cdn-icons.flaticon.com/png/512/3541/premium/3541275.png?token=exp=1650231836~hmac=105442017e95adc8f2fd2041f0be27f5" alt="" />
   <div className=" w-full "><img className='' src={view[0].backdrop} alt="" /> </div>
    ...
    <div className='flex justify-between'><p className="bg-green-600 w-3/6 rounded-3xl" >{view[0].value}</p> </div>
     ...
     <div className=" m-2 justify-around flex">
       <img onClick={()=>units('subtract')} className="w-6 hover:cursor-pointer" src="https://cdn-icons-png.flaticon.com/512/463/463700.png" alt="" /> 
       {unit} <img onClick={()=>units('sum')} className="w-6 hover:cursor-pointer" width={'30px'} src="https://cdn-icons-png.flaticon.com/512/1828/1828817.png" alt ='bao'/>
       <p>{(unit*view[0].value).toFixed(2)}</p>
        <img onClick={()=>{addcart()}} className="w-6 cursor-pointer bg-green-400" src="https://cdn-icons-png.flaticon.com/512/3737/3737372.png" alt="" /></div>
    </div>
   </section>:''}
   {data.length>0?data.map((e)=><div key={e.id} className="p-3  bg-gray-200 ">
    <div className=" w-full "><img className='' src={e.backdrop} alt="" /> </div>
